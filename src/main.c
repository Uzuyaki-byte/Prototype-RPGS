...
int main(void) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == 1) players[i].color = GREEN;
        if (i == 2) players[i].color = GOLD;
    }
    ...
    while (!WindowShouldClose()) {
        ...
        if (!console.active) {
            if (IsKeyDown(KEY_LEFT_ALT)) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    if (CheckCollisionPointRec(mousePos, r)) selectedCharIdx = i;
                }
            } else {
                if (switchingChar) {
                ...
                if (cursorLocked) {
                    ...
                    if (cameraAngleV > 80.0f) cameraAngleV = 80.0f;
                    if (cameraAngleV < -10.0f) cameraAngleV = -10.0f;
                }
                ...
                if (IsKeyPressed(KEY_F)) {
                    ...
                    if (p->inVehicle) {
                        ...
                        p->inVehicle = false;
                        p->position = Vector3Add(vehicles[vIdx].position, (Vector3){ 5, 0, 0 });
                        p->vehicleIdx = -1;
                    } else {
                        int nearest = -1;
                        float minDist = 15.0f;
        ...
        Vector3 targetPos = players[activePlayerIdx].inVehicle ? vehicles[players[activePlayerIdx].vehicleIdx].position : players[activePlayerIdx].position;
        camera.position.x = targetPos.x + cameraDistance * cosf(cameraAngleV * DEG2RAD) * sinf(cameraAngleH * DEG2RAD);
        camera.position.y = targetPos.y + cameraDistance * sinf(cameraAngleV * DEG2RAD);
        camera.position.z = targetPos.z + cameraDistance * cosf(cameraAngleV * DEG2RAD) * cosf(cameraAngleH * DEG2RAD);
        camera.target = targetPos;

        Vector3 forward = Vector3Normalize(Vector3Subtract(targetPos, camera.position));
        forward.y = 0;
        ...
        BeginDrawing();
            ...
            BeginMode3D(camera);
                ...
                DrawMetro(metro);
                for (int i = 0; i < 3; i++) {
                    if (!players[i].inVehicle) DrawPlayer(players[i]);
                }
                for (int i = 0; i < MAX_VEHICLES; i++) {
            ...
            if (switchingChar) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    DrawRectangleRec(r, (i == selectedCharIdx) ? WHITE : (Color){ 200, 200, 200, 150 });
