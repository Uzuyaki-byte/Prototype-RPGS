  ...
  if (strcmp(upper, "FAST") == 0) {
    ...
    for (int j = 0; j < MAX_VEHICLES; j++) {
      if (!vehicles[j].active) {
        ...
        vehicles[j].position =
            (Vector3){player->position.x + 10, 2, player->position.z + 10};
...
void UpdateConsole(CheatConsole *console, Player *player, Vehicle *vehicles) {
  if (IsKeyPressed(KEY_GRAVE)) {
    ...
    if (console->active) {
      EnableCursor();
      // Clear the character buffer so the tilde isn't caught
      while (GetCharPressed() > 0)
        ;
    } else
      DisableCursor();
    return; // Don't process input on the frame it toggles
  }
  ...
  while (key > 0) {
    if ((key >= 32) && (key <= 126) && (key != 96) && (key != 126) &&
        (console->count < MAX_INPUT_CHARS)) {
